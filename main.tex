...
for the saddle point problem with highly variable coefficients, especially the
...
developed by Arnold \textit{et al.}. This kind of preconditioner avoid the
...
\cite{BangerthHartmannKanschat2007}, in particular version 8.1.0, which
provides us with all the building blocks we needed.

\begin{algorithm*}[H]
    \caption{\lstinline$SmootherHdiv::smooth()$}
    \KwData{$\vec{x}_j^0$, $f_j$, $j$}
    \For{$i < m$}{
        Define temporary vector $\tilde{\vec{x}}$\;
        Compute Residual ${\vec{r}}_j^i$\;
        \For{Every vertex} {
            Load the \lstinline$patch_to_global$ map\;
            Extract patch residual ${\vec{r}}_v$\;
            Solve $\hat{A}_v \tilde{\vec{v}}_v = {\vec{r}}_v$\;
            Distribute $\tilde{\vec{x}}_v$ into $\tilde{\vec{x}}$\;
        }
        $\vec{x}_j^{i + 1} = \vec{x}_j^i + \eta \tilde{\vec{x}}$\;
    }
    \Return{$\vec{x}_j^m$}
\end{algorithm*}
\end{block}
